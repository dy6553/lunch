...
const menus: Record<string, string[]> = {
  ...
  '2026-09-02': ['대패연탄불고기덮밥', '다배추된장국', '배추김치', '깻잎무침', '땅콩우엉조림'],
  '2026-09-03': ['발아현미밥', '한우미역국', '매운오리불고기', '볶음파래김자반', '콘치즈구이', '쌈&깻잎묵은지', '새우살애호박볶음'],
  '2026-09-04': ['차조밥', '닭곰탕&당면', '김치전', '육즙떡갈비&파채&요거', '총각김치'],
  ...
  '2026-09-10': ['찹쌀밥', '돼지갈비탕', '고추장비엔나메추리알조림', '방고개회무침', '브라운크로플&엑설런트', '섞박지', '숯불락구이'],
  '2026-09-11': ['후리카케김가루밥', '가쓰오장국', '꼬꼬아찌치킨&사리', '시리얼핫도그', '무피클', '배추김치', '수제청포도에이드', '케요네즈양배추'],
  '2026-09-14': ['미니유부비빔밥', '김치나베우동', '일미볶음', '포크타코', '깍두기', '헬로엔요', '비름나물무침'],
  ...
  try {
    ...
    const writes = Object.entries(menus).map(([date, items]) => ({
      update: {
        ...
        fields: { menuText: { stringValue: items.join('\n') }, date: { stringValue: date } },
      },
      updateMask: { fieldPaths: ['menuText', 'date'] },
      updateTransforms: [{ fieldPath: 'updatedAt', setToServerValue: 'REQUEST_TIME' }],
    }));
    const response = await fetch(`${API_ROOT}/documents:commit`, {
